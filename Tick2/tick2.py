...
def calculate_smoothed_log_probabilities(training_data: List[Dict[str, Union[List[str], int]]], smooth:int = 1) \
    ...
    pos_total = sum(pos.values()) + smooth * len(vocab)
    neg_total = sum(neg.values()) + smooth * len(vocab)
    for pos_token in pos.keys():
        pos[pos_token] = math.log((pos[pos_token] + smooth)/ pos_total)
...
def main():
    ...
    # epochs = 20
    # accuracy_size = np.zeros(shape=(20, 6))
    ...
    # '''Accuracy with smoothing factor'''
    # smoothing_factor = np.linspace(1, 10, 10)
    # epochs = 20
    # accuracy_size = np.zeros(shape=(20, 10))
    # for epoch in range(epochs):
    #     for i, smooth in enumerate(smoothing_factor):
    #         class_spriors = calculate_class_log_probabilities(train_tokenized_data)
    #         smoothed_log_sprobabilities = calculate_smoothed_log_probabilities(train_tokenized_data, smooth)
    #         preds_ssmoothed = []
    #         for review in dev_tokenized_data:
    #             pred = predict_sentiment_nbc(review, smoothed_log_sprobabilities, class_spriors)
    #             preds_ssmoothed.append(pred)
    #         acc_ssmoothed = accuracy(preds_ssmoothed, validation_sentiments)
    #         accuracy_size[epoch, i] = acc_ssmoothed
    
    # plt.figure(figsize=(6, 6))
    # plt.plot(smoothing_factor, accuracy_size.mean(axis=0), linestyle='-', marker='o', color='b', markersize=8)
    # for i, (x, y) in enumerate(zip(smoothing_factor, accuracy_size.mean(axis=0))):
    #     plt.text(x, y + 0.0009, f'({x}, {y:.2f})', fontsize=8, ha='left', va='center')
    # plt.grid(True)
    # plt.xlabel('Smoothing Factor')
    # plt.ylabel('Accuracy')
    # plt.title("Accuracy with Smoothing Factor")
    # plt.savefig("alpha-smoothing.png")
    # plt.show()
